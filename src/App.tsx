...
function App() {
  ...
  const [formattedWeatherData, setFormattedWeatherData] =
    useState<FormattedWeatherData>()
  useEffect(() => {
  ...
  function getSelectedLocation() {
    ...
    if (selectedLocation) {
      const selectedLocationParsed = JSON.parse(
        selectedLocation
      ) as LocationType
      retrieveWeatherData(
        selectedLocationParsed.lon.toFixed(2),
        selectedLocationParsed.lat.toFixed(2),
        selectedLocationParsed.name
      )
    } else {
  ...
  function checkLocationPermission() {
    if (navigator.geolocation) {
      ...
      displayError(
        t('error_location_not_supported') + '||' + t('error_try_sidebar')
      )
    }
  }
  function locationErrorCallback() {
    displayError(t('error_location_cant_get') + '||' + t('error_try_sidebar'))
  }
                ...
                <Switch location={props.location}>
                  <Route exact path="/">
                    <ScreenWeather
                      ...
                      weatherDataFormatted={formattedWeatherData}
                      weatherData={weatherData.standard}
                      reapplyUnitsCallback={reapplyUnits}
                      reloadWeatherDataCallback={getSelectedLocation}
                      changedLocation={getSelectedLocation}
                    />
                  </Route>
                  <Route path="/day/:id">
                    <ScreenDay
