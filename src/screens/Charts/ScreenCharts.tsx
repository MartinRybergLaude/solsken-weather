...
import {
  ...
  Day as FormattedDay,
} from "model/TypesFormattedWeather";
import LineGraph from "components/Graph";
import { AnimatePresence, motion } from "framer-motion";
import { getItem } from "model/utilsStorage";

import { FiArrowLeft } from "react-icons/fi";
import ChoiceChips, { Pair } from "components/ChoiceChips";
...
function ScreenCharts(props: Props) {
  ...
  const [selectedChart, setSelectedChart] = useState<string>("temp");

  useEffect(() => {
    if (
      props.weatherData?.days[props.match.params.id] == null ||
      props.formattedWeatherData?.days[props.match.params.id] == null
    ) {
      props.history.push("/");
    } else {
      var timeout = init(
        props.weatherData.days[props.match.params.id],
        props.formattedWeatherData?.days[props.match.params.id]
      );
    }
    return () => {
  ...
  function getOptionsList(): Pair[] {
    if (getItem("dataSrc") === "smhi") {
      return [
        { key: "temp", value: t("grid_temperature") },
        { key: "wind", value: t("grid_wind") },
        { key: "prec", value: t("grid_prec") },
        { key: "pres", value: t("grid_pressure") },
        { key: "humi", value: t("grid_humidity") },
        { key: "visi", value: t("grid_vis") },
      ];
      ...
      <AnimatePresence>
        {data && (
          ...
          >
            {selectedChart === "temp" && (
              <motion.div
                variants={variantsGraph}
                className={styles.wrapperGraph}
              >
                <LineGraph
                  data={data?.temprData}
                  barType="line"
  ...
  function initVis(day: Day, labels: Array<string>) {
    ...
    return {
      ...
      datasets: [
        {
          label: getItem("dataSrc") === "smhi" ? t("grid_vis") : t("grid_fog"),
          fill: true,
          backgroundColor: "rgba(90, 194, 161, 0.2)",
          borderColor: "rgba(90, 194, 161, 1)",
          data: visList,
